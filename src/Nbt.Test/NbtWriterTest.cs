...
namespace Nbt.Test {
    ...
    public class NbtWriterTest {
        ...
        public void ByteArrayFromStream() {
            ...
            using (var ms = new MemoryStream()) {
                var writer = new NbtWriter(ms, "root");
                {
                    ...
                    writer.BeginList("innerLists", TagType.ByteArray, 4);
                    using (var dataStream = new NonSeekableStream(new MemoryStream(data))) {
        ...
        public void CompoundListTest() {
            ...
            using (var ms = new MemoryStream()) {
                var writer = new NbtWriter(ms, "Test");
                {
                    ...
                    writer.BeginList("ListOfInts", TagType.Int, 3);
                    {
                        writer.WriteInt(1);
                        writer.WriteInt(2);
                        writer.WriteInt(3);
                    ...
                    {
                        writer.BeginList("ListOfCompounds", TagType.Compound, 1);
                        {
                    ...
                    writer.BeginList("ListOfEmptyLists", TagType.List, 3);
                    {
                        writer.BeginList(TagType.List, 0);
                        { }
                        writer.EndList();
                        writer.BeginList(TagType.List, 0);
                        { }
        ...
        public void ListTest() {
            ...
            using (var ms = new MemoryStream()) {
                var writer = new NbtWriter(ms, "Test");
                writer.BeginList("LotsOfLists", TagType.List, 12);
                {
                    writer.BeginList(TagType.Byte, 1);
                    writer.WriteByte(1);
                    writer.EndList();

                    writer.BeginList(TagType.ByteArray, 1);
                    writer.WriteByteArray(new byte[] {
                        1
                    });
                    writer.EndList();

                    writer.BeginList(TagType.Compound, 1);
                    writer.BeginCompound();
                    writer.EndCompound();
                    writer.EndList();

                    writer.BeginList(TagType.Double, 1);
                    writer.WriteDouble(1);
                    writer.EndList();

                    writer.BeginList(TagType.Float, 1);
                    writer.WriteFloat(1);
                    writer.EndList();

                    writer.BeginList(TagType.Int, 1);
                    writer.WriteInt(1);
                    writer.EndList();

                    writer.BeginList(TagType.IntArray, 1);
                    writer.WriteIntArray(new[] {
                        1
                    });
                    writer.EndList();

                    writer.BeginList(TagType.List, 1);
                    writer.BeginList(TagType.List, 0);
                    writer.EndList();
                    writer.EndList();

                    writer.BeginList(TagType.Long, 1);
                    writer.WriteLong(1);
                    writer.EndList();

                    writer.BeginList(TagType.Short, 1);
                    writer.WriteShort(1);
                    writer.EndList();

                    writer.BeginList(TagType.String, 1);
                    writer.WriteString("ponies");
                    writer.EndList();

                    writer.BeginList(TagType.LongArray, 1);
        ...
        public void WriteTagTest() {
            using (var ms = new MemoryStream()) {
                var writer = new NbtWriter(ms, "root");
                {
                    foreach (Tag tag in TestFiles.MakeValueTest().Tags) {
                        writer.WriteTag(tag);
                    }
                    writer.EndCompound();
                    Assert.IsTrue(writer.IsDone);
        ...
        public void ErrorTest() {
            ...
            using (var ms = new MemoryStream()) {
                ...
                var writer = new NbtWriter(ms, "root");
                {
                    ...
                    Assert.Throws<ArgumentOutOfRangeException>(() => writer.BeginList("list", TagType.Int, -1));
                    writer.BeginList("listOfLists", TagType.List, 1);
                    Assert.Throws<ArgumentOutOfRangeException>(() => writer.BeginList(TagType.Int, -1));
                    writer.BeginList(TagType.Int, 0);
                    writer.EndList();
                    writer.EndList();

                    writer.BeginList("list", TagType.Int, 1);

                    // invalid list type
                    Assert.Throws<ArgumentOutOfRangeException>(() => writer.BeginList(TagType.End, 0));
                    Assert.Throws<ArgumentOutOfRangeException>(() => writer.BeginList("list", TagType.End, 0));
                    ...
                    Assert.Throws<NbtFormatException>(() => writer.WriteTag(new IntTag()));
                    Assert.Throws<NbtFormatException>(() => writer.WriteInt(0));
        ...
        public void ComplexStringsTest() {
            ...
            using (var ms = new MemoryStream()) {
                ...
                writer.BeginList("stringList", TagType.String, numStrings);
                ...
                file.LoadFromStream(ms, NbtCompression.None);
                var readStrings =
                    file.RootTag.Get<ListTag>("stringList")
                        .ToArray<StringTag>()
        ...
        public void MissingNameTest() {
            using (var ms = new MemoryStream()) {
                ...
                Assert.Throws<NbtFormatException>(() => writer.WriteTag(new ByteTag(123)));
                Assert.Throws<NbtFormatException>(() => writer.WriteTag(new ShortTag(123)));
                Assert.Throws<NbtFormatException>(() => writer.WriteTag(new IntTag(123)));
                Assert.Throws<NbtFormatException>(() => writer.WriteTag(new LongTag(123)));
                Assert.Throws<NbtFormatException>(() => writer.WriteTag(new FloatTag(123)));
                Assert.Throws<NbtFormatException>(() => writer.WriteTag(new DoubleTag(123)));
                Assert.Throws<NbtFormatException>(() => writer.WriteTag(new StringTag("value")));
                Assert.Throws<NbtFormatException>(() => writer.WriteTag(new ByteArrayTag(new byte[0])));
                Assert.Throws<NbtFormatException>(() => writer.WriteTag(new IntArrayTag(new int[0])));
                Assert.Throws<NbtFormatException>(() => writer.WriteTag(new LongArrayTag(new long[0])));
                Assert.Throws<NbtFormatException>(() => writer.WriteTag(new ListTag(TagType.Byte)));
                Assert.Throws<NbtFormatException>(() => writer.WriteTag(new CompoundTag()));
            }
        }
