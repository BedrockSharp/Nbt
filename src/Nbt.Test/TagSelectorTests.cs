namespace Nbt.Test {
    ...
    public sealed class TagSelectorTests {
        ...
        public void SkippingTagsOnFileLoad() {
            ...
            loadedFile.LoadFromFile(TestFiles.Big,
                                    NbtCompression.None,
                                    tag => tag.TagType != TagType.Float || tag.Parent.Name != "Level");
        ...
        public void SkippingLists() {
            {
                ...
                file.LoadFromBuffer(savedFile, 0, savedFile.Length, NbtCompression.None,
                                    tag => tag.TagType != TagType.List);
                Assert.AreEqual(0, file.RootTag.Count);
            }
            {
                // Check list-compound interaction
                CompoundTag comp = new CompoundTag("root") {
                    new CompoundTag("compOfLists") {
                        new ListTag("listOfComps") {
                            new CompoundTag {
                                new ListTag("emptyList", TagType.Compound)
                            }
                        }
                    }
                };
        ...
        public void SkippingValuesInCompoundTest() {
            CompoundTag root = TestFiles.MakeValueTest();
            CompoundTag nestedComp = TestFiles.MakeValueTest();
